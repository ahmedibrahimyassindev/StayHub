...
class NotificationAccess
{
    ...
    public function requireServiceOrManager(Request $request): ?JsonResponse
    {
        ...
        if (! $identity->canManageNotifications()) {
            return response()->json([
                'message' => 'You are not allowed to manage notifications.',
            ], 403);
        }
    ...
    public function authorizeIndex(Request $request, ?int $requestedRecipientId): JsonResponse|AuthenticatedIdentity|null
    {
        if ($this->isServiceRequest($request)) {
            return null;
        }
        ...
        if ($requestedRecipientId !== null && $requestedRecipientId !== $identity->userId() && ! $identity->canManageNotifications()) {
            return response()->json([
                'message' => 'You are not allowed to access these notifications.',
            ], 403);
        }
    ...
    public function authorizeNotificationRead(Request $request, NotificationMessage $notification): ?JsonResponse
    {
        if ($this->isServiceRequest($request)) {
            return null;
        }
        ...
        if ($notification->recipient_user_id !== $identity->userId() && ! $identity->canManageNotifications()) {
            return response()->json([
